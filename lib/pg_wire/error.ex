defmodule PGWire.Error do
  defmodule ErrorCode do
    # https://github.com/elixir-ecto/postgrex/blob/master/lib/postgrex/error_code.ex
    @moduledoc false
  ...
  @spec fatal(atom(), Keyword.t()) :: t()
  def fatal(error_name, fields \\ [])
  def fatal(error_name, fields), do: new(error_name, "FATAL", fields)

  @spec error(atom(), Keyword.t()) :: t()
  def error(error_name, fields \\ [])
  def error(error_name, fields), do: new(error_name, "ERROR", fields)

  @spec warning(atom(), Keyword.t()) :: t()
  def warning(error_name, fields \\ [])
  def warning(error_name, fields), do: new(error_name, "WARNING", fields)

  @spec new(atom(), binary(), Keyword.t()) :: t()
  def new(error_name, severity, opts) do
    code = ErrorCode.name_to_code(error_name)
    opts = Keyword.merge(opts, severity: severity, code: code)
